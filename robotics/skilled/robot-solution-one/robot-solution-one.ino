...
void loop() {
  ...
  int canGoForward = fDistance > 15 ? 1 : 0;
  int canGoLeft = lDistance > 15 && lDistance > rDistance;
  int canGoRight =  rDistance > 15 && rDistance > lDistance;

  // Work out next turn
  turn = calculateTurn(canGoForward, canGoLeft, canGoRight);
  
  // Make next turn if direction is different
